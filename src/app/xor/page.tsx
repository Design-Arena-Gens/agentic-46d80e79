...
import { generateXOR, normalizePoints, type Point2D } from "@/lib/datasets";
...
export default function XorPage() {
  ...
  useEffect(() => {
    setPoints(normalizePoints(generateXOR(40)));
  }, []);
  ...
  function ensureModel() {
    ...
    m.add(tf.layers.dense({ units: 8, inputShape: [2], activation: "tanh" }));
  ...
  async function trainStep() {
    ...
    const h = await model.current!.fit(X, y, { epochs: 1, batchSize: 32, verbose: 0 });
  ...
  return (
    <div className="space-y-4">
      <h2 className="text-2xl font-semibold">XOR with a Two-Layer MLP</h2>
      <p className="text-sm text-zinc-600 dark:text-zinc-400">Hidden layer tanh, output sigmoid; trained with Adam on log-loss.</p>
      <div className="flex flex-wrap gap-6">
        ...
        <div className="min-w-[260px] space-y-3">
          ...
          <div className="flex gap-2">
            ...
            <button
              onClick={() => setPoints(normalizePoints(generateXOR(40)))}
              className="rounded-md border border-zinc-300 px-3 py-1.5 hover:bg-zinc-100 dark:border-zinc-700 dark:hover:bg-zinc-900"
            >
              New Data
            </button>
